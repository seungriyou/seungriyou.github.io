...
- **겪었던 문제 상황**
    - **임베딩 (embedding)**
        ...
        - 결국, 과제의 설명을 따르는 것이 정답이었다. 과제의 설명에 따르면 `임베딩 == nn.Linear에 넣기` 이며, 그 결과를 그대로 concat 하여 사용하면 되는 것이었다. 따라서 nn.Linear의 두 번째 차원을 1/2씩 줄여서 넣어야 했다.
    - **수많은 런타임 에러 😱**
    ...
    - **activation function**
        ...
        - generator의 마지막 activation function인 `nn.Sigmoid()`를  `nn.Tanh()`로 바꿔보았다. 그랬더니 바로 적절한 generator loss 값과 discriminator loss 값이 관찰되었다. (‼️)
...
- **결과 (epoch 50)** - 깔끔하게 잘 나온다!!!
    ...
    ![](/assets/img/posts/Boostcamp-AI-Tech/Daily-Log/week09/d038-3.png){: width="80%" height="80%"}
